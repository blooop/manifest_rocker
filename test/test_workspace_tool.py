...
def test_workspace_tool():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        workspace_tool = Path(__file__).parent.parent / "scripts" / "workspace"
        shutil.copy(workspace_tool, test_repo / "workspace")

        # Test list command (should show no workspaces)
        result = run_command(["python3", "workspace", "list"], cwd=test_repo)
        assert "No workspaces found" in result.stdout

        print("✓ List command works correctly")

        # Test help command
        result = run_command(["python3", "workspace", "--help"], cwd=test_repo)
